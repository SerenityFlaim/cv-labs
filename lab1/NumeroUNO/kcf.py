...
class Tracker():
    ...
    def get_feature(self, image, roi):
        ...
        # If the box goes completely outside, return a neutral feature
        if x1 >= x2 or y1 >= y2:
            print("Warning: ROI outside image bounds")
            return np.zeros((1, self.ph, self.pw), dtype=np.float32)

        sub_image = image[y1:y2, x1:x2, :]
        if sub_image.size == 0:
            print("Empty sub_image after clipping!")
            return np.zeros((1, self.ph, self.pw), dtype=np.float32)
